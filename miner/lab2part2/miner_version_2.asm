...
WLOOP:
    ...
    //JALR $R30, %SMSIG1
	 //MOV  $R16, $R2
	 SS1  $R16,$R16			// Compute SmallSigma1(W_j-2)	
	 
    //MOV  $R1, $R18
    //JALR $R30, %SMSIG0
    //MOV  $R18, $R2
	 SS0  $R18, $R18			// Compute SmallSigma0(W_j-15)
...
SHAROUND:
	 ...
	 // Compute T1
    MOV  $R19, $R28		// T1 = h
    BS0	$R2, $R25		// Compute BigSig0(e)
	 ADDU $R19, $R2  		// T1 = h + BigSig1(e)
 	 MOV  $R1, $R25
    MOV  $R2, $R26
    MOV  $R3, $R27
    JALR $R30, %CH		// Compute ch(e,f,g)
    ADDU $R19, $R4  		// T1 = h + BigSig1(e) + Ch(e,f,g)
    MOV  $R1, $R29
    ADDU $R1, %K_BASE 	// Compute offset from K
    LW   $R1, $R1
    ADDU $R19, $R1  		// T1 = h + BigSig1(e) + Ch(e,f,g) + K_i
    MOV  $R1, $R29
    ADDU $R1, %W_BASE
    LW   $R1, $R1
    ADDU $R19, $R1  		// T1 = h + BigSig1(e) + Ch(e,f,g) + K_i  W_j

    // Compute T2
    BS1	$R2, $R21		// Compute BigSig1(a)
	 MOV  $R20, $R2 		// T2 = BigSig1(a)
    MOV  $R1, $R21
    MOV  $R2, $R22
    MOV  $R3, $R23
    JALR $R30, %MAJ
    ADDU $R20, $R4		// T2 = SigSig1(a) + Maj(a,b,c) 
...
CH:
    MOV  $R5, $R1
    NOR  $R5, $R5		// ~X
    AND  $R1, $R2  	// $R1 =  X and Y
    AND  $R5, $R3  	// $R5 = ~X and Z
	 XOR	$R5, $R1	 	// $R5 = $R1 XOR $R1
	 MOV	$R4, $R5
    JALR $R31, $R30
...
MAJ:
   MOV  $R5, $R1
   MOV  $R6, $R2
   AND  $R5, $R2 		// $R5 = X AND Y
   AND  $R6, $R3		// $R6 = Y AND Z
   AND  $R1, $R3 		// $R1 = X AND Z
   XOR  $R5, $R6 		// $R5 = $R5 XOR $R6
	XOR  $R5, $R1 		// $R5 = $R5 XOR $R1
	MOV  $R4, $R5
   JALR $R31, $R30
